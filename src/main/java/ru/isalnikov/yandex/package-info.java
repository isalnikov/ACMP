 ...
 *
 *
 * 1. Даем каждому кролику попить из 90 бутылок. Остается 100 неиспитых. 2.1.
 * Кролик умер. Даем каждому кролику по 9 бутылок. Остается 9 неиспитых. 2.1.1
 * Кролик умер. Даем каждому кролику по 1 бутылке и одну оставляем. К концу дня
 * находим бутылку. 2.1.2 Кролики живы. Даем каждому кролику по 1 бутылке. К
 * концу дня находим бутылку. 2.2 Кролики живы. Значит нужно проверить 100
 ...
 * 2.2.2 Кролики живы. Есть 10 бутылок и 10 кроликов. К концу дня находим
 * бутылку.
 *
 */
...
/**
 *
 * //Задача: // //Дано: Кащей Бессмертный, который украл принцессу у Ивана
 * Царевича. И собственно, Иван Царевич, который невесту хочет вернуть. //Кащея
 * убить можно. Есть 10 колодцев с ядом, сила яда от колодцу к колодцу
 * постепенно нарастает. Более сильный яд является противоядием для более
 * слабого (если выпить сначала из 2 колодца, а потом из 3 – отравления не
 * будет). Если смешать два яда, то смесь приобретет свойства сильнейшего. //
 * //Сама битва: соперники приходят на ристалище с одним кубком в руках. Отдают
 * кубок противнику, который выпивает содержимое. Но подлец Кащей построил
 * вокруг последнего 10 колодца крепость, которая неприступна для Ивана. Таким
 * образом, у Кащея есть доступ к сильнейшему яду, а у Ивана нет. // //Цель:
 * победить Кащея и выжить самому.
 *
 *
 * Вторая задача Дано: ящик, в котором гипотетически установлены три лампочки.
 * Почему гипотетически? Потому что ящик закрыт, и, пока вы его не откроете,
 * наличие лампочек не доказуемо. Открыть ящик можно только один раз… Но об этом
 * чуть позже. Итак, закрытый ящик, в котором по условию есть три лампочки. На
 * противоположной от крышки стороне ящика расположены три выключателя, ведущие
 * к лампочкам. Цель – узнать, к какой лампочке ведет каждый из выключателей
 * (какую лампочку какой выключатель включает и выключает). Ящик открыть можно
 * ТОЛЬКО ОДИН РАЗ. После открытия ящик закрывать нельзя. Все эксперименты после
 * открытия ящика приостанавливаются. Фактически, открытие ящика – это знак, что
 * решение найдено и его нужно озвучить. Источник: Devaka.ru
 *
 *
 * Трое мудрецов решили выяснить кто из них умнее и пришли к еще одному умному
 * перцу чтобы тот их рассудил. Тот достал из сундука 5 колпаков – 3 белых и 2
 * черных, потом завязал троим мудрецам глаза и надел на них по колпаку. Следом
 * – снял повязки и предложил им угадать какой колпак на них одет (при этом
 * каждый из мудрецов не видит своего колпака, но видит что одето на
 * оппонентов). Через некоторое время один из мудрецов заявил что на нем белый
 * колпак – и выиграл состязание. Источник: Devaka.ru
 *
 *
 *
 *
 * Задача 1. В последовательности записаны целые числа от 1 до N в произвольном
 * порядке, но одно из чисел пропущено (остальные встречаются ровно по одному
 * разу). N заранее неизвестно. Определить пропущенное число
 *
 *
 * Задача 2. Определить 2 числа ?
 *
 *
 */
